...
type RestOf<T extends IContract | NewContract> = Omit<T, 'contractDate'> & {
...
export class ContractService {
  ...
  protected convertDateFromClient<T extends IContract | NewContract | PartialUpdateContract>(contract: T): RestOf<T> {
    return {
      ...contract,
      contractDate: contract.contractDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restContract: RestContract): IContract {
    return {
      ...restContract,
      contractDate: restContract.contractDate ? dayjs(restContract.contractDate) : undefined,
    };
  }
